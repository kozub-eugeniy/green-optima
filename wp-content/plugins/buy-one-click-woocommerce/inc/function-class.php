...
class BuyFunction {
    ...
    static function viewBuyForm($cartinfo) {
        ...
        if (!empty($cartinfo['imageurl'])) {
            $imagetovar = '<img src="' . $cartinfo['imageurl'] . '">'; // Изображение
        } else {
            $imagetovar = '';
        }
        ?>
        <div id="formOrderOneClick">
            ...
            <div class="popup popup-form">
                ...
                <form class="b1c-form" method="post" action="#" id="contactform">
                   ...
                   <h2><?php echo $nametovar; ?></h2>

<div class="one-right">
<p style="font-size: 14px;color: black;font-family: OpenSansLight;">Менеджер перезвонит Вам, узнает все детали и сам оформит заказ на Ваше имя.</p>
                    <?php if (!empty(BuyCore::$buyoptions['fio_chek'])) { ?>
                        ...
                        <input class="buyvalide" type="tel" pattern="/^[0-9]{7,10}$" <?php
                    ...
                    <input type="hidden" name="buy_idtovar" value="<?php echo $idtovar; ?>" /> 
                             ...
                             <div class="one1"><span class="descrip">Цена: </span><span class="descrip1"><?php echo $pricetovar; ?>  грн.</span></div>
    ...
    static function viewBuyButton() {
        if (!empty(BuyCore::$buyoptions['namebutton']) and ! empty(BuyCore::$buyoptions['positionbutton'])) {
            ...
            <a class="clickBuyButton button21" href="#" data-productid="<?php echo $product->id  ; ?>"><?php echo BuyCore::$buyoptions['namebutton']; ?></a>
    ...
    static function viewBuyButtonCustrom($arParams) {
        if (!empty(BuyCore::$buyoptions['namebutton']) and ! empty(BuyCore::$buyoptions['positionbutton'])) {
            ...
            <a class="clickBuyButtonCustom button21" href="#" data-productid="<?php echo $arParams['id']; ?>" data-name="<?php echo $arParams['name']; ?>" data-count="<?php echo $arParams['count']; ?>" data-price="<?php echo $arParams['price']; ?>"><?php echo BuyCore::$buyoptions['namebutton']; ?></a>
    ...
    static function BuyInfoCart($productid, $urlpost) {
        ...
        if (method_exists($product, 'get_image_id')) {
            ...
            $imageurl = wp_get_attachment_image_src($product->get_image_id(), 'full'); //Урл картинки товара
