...
if (!function_exists('woof_print_tax'))
{
    ...
    function woof_print_tax($taxonomies, $tax_slug, $terms, $exclude_tax_key, $taxonomies_info, $additional_taxes, $woof_settings, $args, $counter)
    {
        ...
        ?>
        <div data-css-class="woof_container_<?php echo $tax_slug ?>" class="sidebar-filter woof_container woof_container_<?php echo $woof_settings['tax_type'][$tax_slug] ?> woof_container_<?php echo $tax_slug ?> woof_container_<?php echo $counter ?> woof_container_<?php echo $primax_class ?>">
                ...
                switch ($woof_settings['tax_type'][$tax_slug])
                {
                    case 'checkbox':
                        if ($WOOF->settings['show_title_label'][$tax_slug])
                        {
                            ...
                            <span class="sidebar-filter__title"><?php echo WOOF_HELPER::wpml_translate($taxonomies_info[$tax_slug]) ?><?php WOOF_HELPER::draw_title_toggle($show_toggle, $block_is_closed); ?></span>
                            <?php if($counter == 1 && !empty($_GET)){ ?>
                                <span class="sidebar-filter__clear" onclick="window.location=window.location.origin+window.location.pathname">Очистить</span>
                            <?php } ?>
                            </div>
                            <?php
                        }
        ...
        switch ($key)
        {
            case 'by_price':
                ...
                <?php if ($price_filter == 4): ?>
                    <div data-css-class="woof_price4_search_container" class="woof_price4_search_container woof_container sidebar-filter">
                        <div class="woof_container_overlay_item"></div>
                        <div class="woof_container_inner">
                            <?php if (isset($WOOF->settings['by_price']['title_text']) AND ! empty($WOOF->settings['by_price']['title_text'])): ?>
                                <<?php echo apply_filters('woof_title_tag', 'h4'); ?>><?php echo WOOF_HELPER::wpml_translate(null, $WOOF->settings['by_price']['title_text']); ?></<?php echo apply_filters('woof_title_tag', 'h4'); ?>>
