	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		switch ( $args['type'] ) {
			...
			case 'textarea' :
				...
				$field .= '<textarea name="' . esc_attr( $key ) . '" class="input-text order-form__text' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '" ' . ( empty( $args['custom_attributes']['rows'] ) ? ' rows="2"' : '' ) . ( empty( $args['custom_attributes']['cols'] ) ? ' cols="5"' : '' ) . implode( ' ', $custom_attributes ) . '>' . esc_textarea( $value ) . '</textarea>';
